...
function Mesh({ model, large = false }: { model: Model; large?: boolean }) {
  return (
    <div className={`mesh mesh-${model.poly} ${large ? "mesh-large" : ""}`} style={{ "--accent": model.color } as React.CSSProperties}>
      <div className="mesh-orbit orbit-a" />
      <div className="mesh-orbit orbit-b" />
      <div className="mesh-core"><i /><i /><i /><i /><i /></div>
...
export default function Home() {
  ...
  const filtered = filter === "全部" ? models : models.filter((m) => m.type === filter);
    ...
    <main>
      ...
      {tab === "overview" && <>
        <section className="hero">
          ...
          <div className="hero-visual"><Mesh model={models[0]} large /><div className="scan-line" /><div className="callout c1"><span>01</span> 肩部环线连续</div><div className="callout c2"><span>02</span> 肘部预留 3 环</div><div className="callout c3 warning"><span>!</span> 腰侧密度偏高</div><div className="visual-caption">KAI_HERO_V23.FBX <b>LIVE VIEW</b></div></div>
        </section>
      ...
      {tab === "models" && <section className="workspace-section">
        <div className="page-title"><div><span className="index">MODEL LIBRARY / 06 ASSETS</span><h1>模型浏览</h1><p>从资产库选择模型，查看基础数据与各维度质量表现。</p></div><button className="primary" onClick={() => notify("上传入口已就绪（Demo）")}>＋ 上传模型</button></div>
        <div className="filters">{["全部", "角色", "载具", "场景", "道具"].map(f => <button key={f} className={filter === f ? "active" : ""} onClick={() => setFilter(f)}>{f}</button>)}</div>
        <div className="model-layout"><div className="model-grid">{filtered.map(m => <button key={m.id} className={`model-card ${selected.id === m.id ? "selected" : ""}`} onClick={() => setSelected(m)}><div className="model-preview"><Mesh model={m} /><ScoreRing score={m.score} small /></div><div className="model-info"><div><b>{m.name}</b><span>{m.type} · FBX</span></div><em style={{ color: m.color }}>{m.tris} tris</em></div></button>)}</div>
          <aside className="model-detail"><span className="side-label">SELECTED ASSET</span><h2>{selected.name}</h2><div className="detail-mesh"><Mesh model={selected} large /></div><div className="spec-row"><span>三角面<b>{selected.tris}</b></span><span>顶点<b>{selected.verts}</b></span><span>贴图<b>{selected.texture}</b></span></div><h3>维度表现</h3>{selected.quality.map((q, i) => <div className="quality-row" key={labels[i]}><span>{labels[i]}</span><i><b style={{ width: `${q}%`, background: selected.color }} /></i><em>{q}</em></div>)}<button className="primary full" onClick={() => { setScores(selected.quality); navigate("evaluate"); }}>评测此模型 →</button></aside>
        </div>
      </section>}
      ...
      {tab === "evaluate" && <section className="workspace-section">
        ...
        <div className="evaluation-layout"><div className="viewer-panel"><div className="viewer-toolbar"><span>{selected.name}.FBX</span><div>{["实体", "线框", "法线"].map(v => <button key={v} className={view === v ? "active" : ""} onClick={() => setView(v)}>{v}</button>)}</div></div><div className={`eval-mesh view-${view}`}><Mesh model={selected} large /><span className="viewer-note">拖动旋转 · 滚轮缩放</span></div><div className="viewer-stats"><span>TRIS <b>{selected.tris}</b></span><span>VERTS <b>{selected.verts}</b></span><span>MATERIALS <b>03</b></span></div></div>
      ...
      {tab === "pk" && <section className="workspace-section">
        ...
        <div className="pk-stage"><article style={{ "--model-color": left.color } as React.CSSProperties}><div className="pk-label">MODEL A</div><Mesh model={left} large /><h2>{left.name}</h2><div className="pk-specs"><span>{left.tris}<small>TRIS</small></span><span>{left.verts}<small>VERTS</small></span><ScoreRing score={left.score} small /></div></article><div className="pk-divider"><span>VS</span></div><article style={{ "--model-color": right.color } as React.CSSProperties}><div className="pk-label">MODEL B</div><Mesh model={right} large /><h2>{right.name}</h2><div className="pk-specs"><span>{right.tris}<small>TRIS</small></span><span>{right.verts}<small>VERTS</small></span><ScoreRing score={right.score} small /></div></article></div>
